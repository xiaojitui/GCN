...
class GCNModel(object):
    ...
    def __init__(params):
        ...
        self.p.save_dir = '{}/{}'.format(self.p.model_dir, self.p.name) #'./models/test/'
        
        if not os.path.exists(self.p.log_dir):
            os.system('mkdir -p {}'.format(self.p.log_dir))
        if not os.path.exists(self.p.save_dir): 
            os.system('mkdir -p {}'.format(self.p.model_dir))
...
# ## add all following into the GCNModel class
...
def load_data(self):
        ...
        print("loading data")
        self.data = {}
        self.A, self.X, self.data['y_train'], self.data['y_valid'], self.data['y_test'],         self.data['mask_train'], self.data['mask_valid'], self.data['mask_test'] = load_network(self.p.dataset)
