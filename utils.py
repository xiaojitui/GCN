...
def load_network(dataset_str):
    ...
    features = sp.vstack((allx, tx)).tolil() # train and test. list of list
    features[test_idx_reorder, :] = features[test_idx_range, :]
    adj = nx.adjacency_matrix(nx.from_dict_of_lists(graph))

    labels = np.vstack((ally, ty)) # train and test
...
def preprocess_features(features):
    ...
    ## same as each row / row sum: s[i]/s.sum(1)[i]
    
    rowsum = np.array(features.sum(1))
    r_inv = np.power(rowsum, -1).flatten()
    r_inv[np.isinf(r_inv)] = 0.   #1/rowsum
    r_mat_inv = sp.diags(r_inv) #diag of 1/rowsum
    features = r_mat_inv.dot(features) # diag of 1/rowsum - matdot - feature
